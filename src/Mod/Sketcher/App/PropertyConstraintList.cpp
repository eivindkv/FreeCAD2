...
{
    return App::ObjectIdentifier(getContainer()) << App::ObjectIdentifier::Component::ArrayComponent(getName(), idx);
}
...
App::ObjectIdentifier PropertyConstraintList::makeSimplePath(const Constraint * c)
{
    return App::ObjectIdentifier(getContainer()) << App::ObjectIdentifier::Component::SimpleComponent(getName())
                           << App::ObjectIdentifier::Component::SimpleComponent(c->Name);
}
...
void PropertyConstraintList::applyValues(const std::vector<Constraint*>& lValue)
{
    ...
    for (unsigned int i = 0; i < lValue.size(); i++) {
        boost::unordered_map<boost::uuids::uuid, int>::const_iterator j = valueMap.find(lValue[i]->tag);
    ...
    for (unsigned int i = 0; i < oldVals.size(); i++) {
        boost::unordered_map<boost::uuids::uuid, int>::const_iterator j = valueMap.find(oldVals[i]->tag);
...
void PropertyConstraintList::setValue(const ObjectIdentifier &path, const boost::any &value)
{
    ...
    if (c0.isArray() && path.numSubComponents() == 1) {
        ...
        aboutToSetValue();
        _lValueList[c0.getIndex()]->Value = dvalue;
        hasSetValue();
        return;
    }
        ...
        for (std::vector<Constraint *>::const_iterator it = _lValueList.begin(); it != _lValueList.end(); ++it) {
            if ((*it)->Name == c1.getName()) {
                aboutToSetValue();
                _lValueList[it - _lValueList.begin()]->Value = dvalue;
                hasSetValue();
                return;
            }
        }
...
{
    return boost::any(getConstraint(path)->Value);
}
...
const ObjectIdentifier PropertyConstraintList::canonicalPath(const ObjectIdentifier &p) const
{
    ...
    if (c0.isArray() && p.numSubComponents() == 1) {
        ...
        if (_lValueList[c0.getIndex()]->Name.size() > 0)
