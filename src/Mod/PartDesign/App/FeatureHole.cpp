...
{
    Subtractive::onChanged(prop);
    if (prop == &ThreadType) {
        if (ThreadType.getValueAsString() == "None" ) {
            ThreadSize.setEnums(ThreadSize_None_Enums);
            ThreadClass.setEnums(ThreadClass_None_Enums);
            HoleCutType.setEnums(HoleCutType_None_Enums);
        }
        else if ( ThreadType.getValueAsString() == "ISOMetricProfile" ) {
        ...
        else if ( ThreadType.getValueAsString() == "ISOMetricFineProfile" ) {
        ...
        else if ( ThreadType.getValueAsString() == "UNC" ) {
        ...
        else if ( ThreadType.getValueAsString() == "UNF" ) {
        ...
        else if ( ThreadType.getValueAsString() == "UNEF" ) {
...
{
    App::DocumentObject* obj = Sketch.getValue();

    if (!obj)
        throw Base::Exception("No sketch linked");
    if (!obj->getTypeId().isDerivedFrom(Sketcher::SketchObject::getClassTypeId()))
        throw Base::Exception("Linked object is not a Sketch");

    Sketcher::SketchObject* sketch = static_cast<Sketcher::SketchObject*>(obj);

    // if the Base property has a valid shape, cut the holes from it
    TopoDS_Shape base;
    try {
        base = getBaseShape();
    ...
    try {
        ...
        std::vector<Part::Geometry*> parts = sketch->getCompleteGeometry();
        BRep_Builder builder;
        TopoDS_Compound holes;

        builder.MakeCompound(holes);

        for (std::vector<Part::Geometry*>::iterator i = parts.begin(); i != parts.end(); ++i) {
            Part::Geometry* part = *i;
            double length;

            // Skip if geomerty is not a circle
            if ( !part->getTypeId().isDerivedFrom( Part::GeomCircle::getClassTypeId() ) )
                continue;
            ...
            if ( method == "Dimension" )
                ...
                // Use large (10m), but finite length
                length = 1e4;
            }
            else
            ...
            bool countersink = (holeCutType == "Countersink" || holeCutType == "Countersink socket screw");
            bool counterbore = (holeCutType == "Counterbore" || holeCutType == "Cheesehead" || holeCutType == "Cap screw");
            double taperedAngle = Tapered.getValue() ? Base::toRadians( TaperedAngle.getValue() ) : Base::toRadians(90.0);
            double radiusBottom = Diameter.getValue() / 2.0 - length * cos( taperedAngle );
            ...
            if ( taperedAngle <= 0 || taperedAngle > Base::toRadians( 180.0 ) )
            ...
            if ( countersink ) {
                ...
                // Top point
                gp_Pnt newPoint = gp_Pnt(holeCutRadius, 0, 0);
                ...
                // Top point
                gp_Pnt newPoint(holeCutRadius, 0, 0);
                mkWire.Add(BRepBuilderAPI_MakeEdge(lastPoint, newPoint));
                lastPoint = newPoint;

                // Bottom of counterbore
                newPoint = gp_Pnt(holeCutRadius, 0, -holeCutDepth);
                mkWire.Add(BRepBuilderAPI_MakeEdge(lastPoint, newPoint));
                lastPoint = newPoint;
            ...
            if (drillPoint == "Flat") {
                gp_Pnt newPoint = gp_Pnt(radiusBottom, 0, -length);
                mkWire.Add(BRepBuilderAPI_MakeEdge(lastPoint, newPoint));
                lastPoint = newPoint;

                newPoint = gp_Pnt(0, 0, -length);
                mkWire.Add( BRepBuilderAPI_MakeEdge( lastPoint, newPoint ) );
                lastPoint = newPoint;
            }
            ...
            BRepPrimAPI_MakeRevol RevolMaker(face, gp_Ax1(firstPoint, direction), angle);

            if (RevolMaker.IsDone()) {
                TopoDS_Shape hole = RevolMaker.Shape();

                if (hole.IsNull())
                    return new App::DocumentObjectExecReturn("Hole: Resulting shape is empty");

                Handle_Geom_Circle circle = Handle_Geom_Circle::DownCast(part->handle());
                ...
                BRepBuilderAPI_Transform transformer(hole, sketchTransformation * localSketchTransformation);
        ...
        this->SubShape.setValue( holes );

        // Cut the SubShape out of the base feature
        BRepAlgoAPI_Cut mkCut( base, holes );
        if (!mkCut.IsDone())
