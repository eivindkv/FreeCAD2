...
std::vector<App::DocumentObject*> ViewProviderHole::claimChildren(void)const
{
    ...
    temp.push_back(static_cast<PartDesign::Hole*>(getObject())->Sketch.getValue());
...
bool ViewProviderHole::onDelete(const std::vector<std::string> &s)
{
    ...
    if (pcHole->Sketch.getValue())
        pcSketch = static_cast<Sketcher::SketchObject*>(pcHole->Sketch.getValue());
