...
void Document::exportGraphviz(std::ostream& out) const
{
    ...
    private:
        ...
        void addEdges() {
            ...
            std::set<std::pair<DocumentObject*,DocumentObject*> > existingEdges;

            // Add edges between properties
            std::set<DocumentObject*>::const_iterator j = objects.begin();
            while (j != objects.end()) {
                DocumentObject * docObj = *j;
                ...
                while (i != expressions.end()) {
                    ...
                    while (k != deps.end()) {
                        ...
                        existingEdges.insert(std::make_pair<DocumentObject*, DocumentObject*>(*j, depObjDoc));
            ...
            for (std::map<std::string, DocumentObject*>::const_iterator It = d->objectMap.begin(); It != d->objectMap.end();++It) {
                ...
                for (std::vector<DocumentObject*>::const_iterator It2=OutList.begin();It2!=OutList.end();++It2) {
                    if (*It2) {
                        ...
                        if (edge(GlobalVertexList[getId(It->second)], GlobalVertexList[getId(*It2)], DepList).second)
                            continue;

                        // Skip edge if an expression edge already exists
                        if (existingEdges.find(std::make_pair<DocumentObject*, DocumentObject*>(It->second, *It2)) != existingEdges.end())
                            continue;
                        ...
                        tie(edge, inserted) = add_edge(GlobalVertexList[getId(It->second)], GlobalVertexList[getId(*It2)], DepList);

                        // Set properties to make arrows go between subgraphs if needed
                        if (GraphList[It->second])
                            edgeAttrMap[edge]["ltail"] = getClusterName(It->second);
                        if (GraphList[*It2])
        ...
        std::set<DocumentObject*> objects;
