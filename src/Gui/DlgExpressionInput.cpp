    ...
    impliedUnit(_impliedUnit)
{
    ...
    if (expression) {
        ...
        textChanged(QString::fromStdString(expression->toString()));
    }
    ...
    App::Document * doc = path.getDocument();
    QCompleter * completer = new ExpressionCompleter(doc, docObj, this);

    ui->expression->setCompleter(completer);
...
{
    try {
        boost::shared_ptr<Expression> expr(ExpressionParser::parse(path.getDocumentObject(), text.toStdString().c_str()));
        ...
        if (expr) {
...
#ifdef FC_DEBUG
            ui->parsedExpr->setText(QString::fromStdString(expr->toString()));
#endif
            ...
            if (n) {
                ...
                ui->result->setText(QString::fromStdString(result->toString()));
        }
    }
